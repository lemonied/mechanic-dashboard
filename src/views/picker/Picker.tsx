...
let cropData: any = { width: 200, height: 200 };
...
const Picker = forwardRef<PickerInstance | undefined, Props>((props, ref) => {
    ...
    if (cropperRef.current) {
      ...
      cropperRef.current = new Cropper(imageRef.current!, {
        ...
        ready(event: Cropper.ReadyEvent<HTMLImageElement>) {
          const crop = (this as any).cropper as Cropper;
          if (canvasData) {
            crop.setCanvasData(canvasData);
          }
          if (cropData) {
            crop.setData(cropData);
          }
        },
      });
    }
  ...
  const savePosition = useCallback(() => {
    ...
    canvasData = cropperRef.current?.getCanvasData();
  }, []);
  ...
  }, [onSubmit, getCroppedData, savePosition]);
